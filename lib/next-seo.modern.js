...
const buildTags = config => {
          ...
          _extends(
            {
              key: `meta:${
                (_ref2 =
                  (_ref3 = keyOverride != null ? keyOverride : tag.name) != null
                    ? _ref3
                    : tag.property) != null
                  ? _ref2
                  : tag.httpEquiv
  ...
  if (
    ...
    config.additionalLinkTags.forEach(tag => {
      ...
      const { crossOrigin: tagCrossOrigin } = tag,
...
function CarouselJsonLd(_ref) {
  let { type = 'Carousel', keyOverride, ofType, data } = _ref,
...
function BreadCrumbJsonLd(_ref) {
  let { type = 'BreadcrumbList', keyOverride, itemListElements } = _ref,
...
function ParkJsonLd(_ref) {
  let { address, geo, images, keyOverride, openingHours, type = 'Park' } = _ref,
